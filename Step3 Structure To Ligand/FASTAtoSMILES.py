...
class LigandPostprocessor:
    ...
    #         print("Found existing mapping file, now reading ...")
    #         with open(mapping_file, 'r') as f:
    #             reader = csv.reader(f)
    #             # The original logic here might change since we no longer save indexes.
    #             for row in reader:
    #                 self.hash_ligand_mapping[row[0]] = row[1]
    #             # Update based on the list length if SMILES are stored directly.
    #             self.current_index = len(self.hash_ligand_mapping)

    def save_mapping(self):
        # This method is updated to only save SMILES strings, not the indexes.
        mapping_file = os.path.join(self.output_path, 'Stucture.smi')  # Optionally changed the file name
        with open(mapping_file, 'w', newline='') as f:
            for ligand in self.hash_ligand_mapping.values():
                f.write(ligand + "\n")  # Write each SMILES in a new line
    ...
    def to_sdf(self, ligand_list_per_batch):
        ...
        # hash_ligand_mapping_per_batch = {} # 之前使用该字典将不再需要
        for ligand in tqdm(ligand_list_per_batch):
            # 检查SMILES字符串的长度和有效性
            if len(ligand) > 100:
                continue
            mol = Chem.MolFromSmiles(ligand)
            if mol is None:  # 如果无效，则跳过
                continue

            filepath = os.path.join(self.output_path, str(self.current_index) + '.sdf')
            ...
            if os.path.exists(filepath):
                # 使用数字索引而非哈希值记录SMILES串
                self.hash_ligand_mapping[str(self.current_index)] = ligand
                self.current_index += 1  # 递增索引
    ...
    def delete_all_sdf_files(self):
        print("Deleting all .sdf files...")
        for sdf_file in os.listdir(self.output_path):
            if sdf_file.endswith('.sdf'):
                os.remove(os.path.join(self.output_path, sdf_file))
                print(f"Deleted {sdf_file}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--pro_seq', type=str, default=None,
    ...
    parser.add_argument('-l', '--ligand_prompt', type=str, default='',
                        help='Input a ligand prompt. Default value is an empty string.')
    parser.add_argument('-e', '--empty_input', action='store_true', default=False,
    ...
    parser.add_argument('-o', '--output', type=str, default='./structure_output',
                        help="Output directory for generated molecules. Default value is './structure_output/'.")
    parser.add_argument('-b', '--batch_size', type=int, default=32,
                        help="How many molecules will be generated per batch. Try to reduce this value if you have low RAM. Default value is 32.")
    parser.add_argument('--top_k', type=int, default=9,
                        help='The number of highest probability tokens to consider for top-k sampling. Defaults to 9.')
    parser.add_argument('--top_p', type=float, default=0.9,
                        help='The cumulative probability threshold (0.0 - 1.0) for top-p (nucleus) sampling. It defines the minimum subset of tokens to consider for random sampling. Defaults to 0.9.')
    parser.add_argument('-s', '--save_sdfs', action='store_true', default=False,
                        help='Save .sdf files after generation. Default is to not save.')
    ...
    while ligand_post_processor.current_index < num_generated:
        ...
        for sample_output in sample_outputs:
            ...
            if directly_gen:
                directly_gen_protein_list.append(
                    tokenizer.decode(sample_output, skip_special_tokens=True).split('<L>')[0])
    ...
    if not save_sdfs:
        ligand_post_processor.delete_all_sdf_files()
