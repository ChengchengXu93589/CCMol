...
def crossover_non_ring(parent_A, parent_B):
    for i in range(10):
        ...
        for fa in fragments_A:
            for fb in fragments_B:
                reaction_products = rxn.RunReactants((fa,fb))
                if reaction_products:
                    new_mol_trial.append(reaction_products[0])
...
def crossover(parent_A, parent_B):
    if parent_A is None or parent_B is None:
        # 处理 parent_A 或 parent_B 为 None 的情况
        return None
    parent_smiles = [Chem.MolToSmiles(parent_A), Chem.MolToSmiles(parent_B)]
    try:
        Chem.Kekulize(parent_smiles[0], clearAromaticFlags=True)
        Chem.Kekulize(parent_smiles[1], clearAromaticFlags=True)
    except:
        pass
    for i in range(10):
        if random.random() <= 0.5:
